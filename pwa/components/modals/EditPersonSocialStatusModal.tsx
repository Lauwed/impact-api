import { useEffect, useState } from "react";
import { PersonSocialStatus } from "../../types";
import Button from "../Button";
import Heading from "../common/Heading";
import { useAuth } from "../context/auth";
import Label from "../form/Label";
import Modal from "../Modal";
import SourcesSelector from "../selectors/SourcesSelector";
import TypeSocialStatusesSelector from "../selectors/TypeSocialStatusesSelector";
import AddSourceModal from "./AddSourceModal";

const EditPersonSocialStatusModal = ({
  modalOpen,
  setModalOpen,
  onClose,
  socialStatus,
}: {
  modalOpen: boolean;
  setModalOpen: (state: boolean) => void;
  onClose?: () => void;
  socialStatus: PersonSocialStatus;
}) => {
  const { user } = useAuth();
  const [isSourceModalOpen, setIsSourceModalOpen] = useState(false);
  const [formData, setFormData] = useState({
    typeSocialStatus: socialStatus.typeSocialStatus.id,
    source: socialStatus.source.id,
  });

  useEffect(() => {
    if (!modalOpen && onClose) {
      onClose();
    }
  }, [modalOpen]);

  const handleChange = (e: any) => {
    const { name, value } = e.target;
    setFormData((prevData) => ({
      ...prevData,
      [name]: value,
    }));
  };

  const handleSubmit = async (e: any) => {
    e.preventDefault();
    try {
      const response = await fetch(
        `/person_social_statuses/${socialStatus.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/ld+json",
            Authorization: `Bearer ${user?.token}`,
          },
          body: JSON.stringify({
            ...formData,
            typeSocialStatus: `/type_social_statuses/${formData.typeSocialStatus}`,
            source: `/sources/${formData.source}`,
            person: socialStatus.person,
          }),
        }
      );

      if (response.ok) {
        const data = await response.json();
        console.info("Added successfully", data);
        setModalOpen(false);
      } else {
        console.error("Request failed");
      }
    } catch (error) {
      console.error("Error:", error);
    }
  };

  return (
    <Modal isOpen={modalOpen} setIsOpen={setModalOpen}>
      <Heading level="h3">Edit a social status information</Heading>
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <Label htmlFor="typeSocialStatus">Type social status</Label>
          <TypeSocialStatusesSelector
            value={formData.typeSocialStatus}
            onChange={handleChange as () => void}
          />
        </div>

        <div className="mb-4">
          <Label htmlFor="source">Source</Label>
          <SourcesSelector
            value={formData.source}
            onChange={(value: number) => {
              setFormData((prevData) => ({
                ...prevData,
                source: value,
              }));
            }}
          />

          <div className="flex gap-4 items-center mt-2">
            <p>The source doesn't exists ?</p>
            <Button type="button" onClick={() => setIsSourceModalOpen(true)}>
              Add a source
            </Button>
          </div>
        </div>

        <Button type="submit">Edit social status</Button>
      </form>

      <AddSourceModal
        modalOpen={isSourceModalOpen}
        setModalOpen={setIsSourceModalOpen as () => void}
      />
    </Modal>
  );
};

export default EditPersonSocialStatusModal;
