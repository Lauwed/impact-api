...
export function EditableSection({ 
  ...
  const handleChange = (index: number, field: 'type' | 'value', value: number | string) => {
  ...
  const renderKeyInput = (item: Item, index: number) => {
    ...
    if (typeOptions.length > 0) {
      return (
        <Select
          ...
          onValueChange={(value) => handleChange(index, 'type', parseInt(value, 10))}
        >
    ...
    return (
      <Input
        value={item.type?.toString() || ''}
        onChange={(e) => handleChange(index, 'type', parseInt(e.target.value, 10))}
        className="flex-grow"
        placeholder="Type"
        type="number"
      />
          ...
          <li key={item.id || index} className="flex items-center space-x-2">
            {editingIndex === index ? (
              <>
                ...
                <Button size="icon" variant="outline" onClick={() => handleCancel(index)}>
                  <X className="h-4 w-4" />
                </Button>
              </>
            ) : (
